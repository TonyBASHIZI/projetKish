<?php
    ...
    {
       $sql = "SELECT bag.id as CODECMD,bag.userID as REFCl,bag.oder_date as DATECMD,t_client.nom AS NOM,t_client.tel AS TEL,t_client.email AS EMAIL,bag_detail.qty as QTE, t_approvision.design_produit as CODEPRO,t_approvision.detail AS DESIGNPRO,t_approvision.prix_ui as PRIX_UI, (bag_detail.qty * t_approvision.prix_ui) as TOTAL, bag.statut as ETAT FROM `bag` INNER JOIN t_client on bag.userID = t_client.code_client
       INNER JOIN bag_detail on bag.id = bag_detail.bagID INNER JOIN t_approvision on bag_detail.productID = t_approvision.id ";
       $results = mysqli_query($mysqli,$sql); 
    }else{
        
       $sql = "SELECT bag.id as CODECMD,bag.userID as REFCl,bag.oder_date as DATECMD,t_client.nom AS NOM,t_client.tel AS TEL,t_client.email AS EMAIL,bag_detail.qty as QTE, t_approvision.design_produit as CODEPRO,t_approvision.detail AS DESIGNPRO,t_approvision.prix_ui as PRIX_UI, (bag_detail.qty * t_approvision.prix_ui) as TOTAL, bag.statut as ETAT FROM `bag` INNER JOIN t_client on bag.userID = t_client.code_client
       INNER JOIN bag_detail on bag.id = bag_detail.bagID INNER JOIN t_approvision on bag_detail.productID = t_approvision.id  WHERE date_Format(oder_date, '%Y-%m-%d') = date_Format(Now(), '%Y-%m-%d') ";
       $results = mysqli_query($mysqli,$sql); 
    }
  ...
  <body onload="print()">
    <section class="page--header">
     <div class="col-md-8">
                    ...
                    <div class="panel">
                    <div class="records--list" data-title="recentes commandes des clients * commandes journalieres">
